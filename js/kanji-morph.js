...
class KanjiMorph {
  ...
  prepareMorph() {
    ...
    for (let i = 0; i < this.currentKanji.strokes.length; i++) {
      ...
      let addedStroke = this.targetKanji.strokes[i]

      let firstStrokeLen = stroke.segments.length // get the length before we change it with join()
      stroke.join(addedStroke)

      let firstStrokeEnd = stroke.segments[firstStrokeLen - 1]
      firstStrokeEnd.handleOut = firstStrokeEnd.handleIn * -1
      firstStrokeEnd.handleOut.length = Math.max(firstStrokeEnd.handleOut.length, KanjiMorph.HandleMin)
