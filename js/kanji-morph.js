...
class KanjiMorph {
  ...
  startMorph(targetChar) {
    if (!this.currentKanji) {
      console.error("startMorph called before initial kanji loaded")
      return
    }
    ...
    for (let i = 0; i < this.currentKanji.live.strokes.length; i++) {
      ...
      if (strokeDiff > 0) {
        for (let i = 0; i < strokeDiff; i++) {
          let strokeHeadPos = currStroke.segments[0].point
          currStroke.insert(0, strokeHeadPos)
        }
      } else if (strokeDiff < 0) {
        for (let i = 0; i < Math.abs(strokeDiff); i++) {
          let strokeHeadPos = targetStroke.segments[0].point
          targetStroke.insert(0, strokeHeadPos)
        }
      }
  ...
  step(frameEvent) {
    ...
    if (!this.tStarted) {
      // first frame of morph, map and split strokes
      this.tStarted = t
...
function onKeyDown(event) {
  if (event.key == 'space') {
    let randomKanji = supportedKanji[Math.floor(Math.random() * supportedKanji.length)]
    kanjiMorph.startMorph(randomKanji)
  }

  // multiple morphs in succession with startMorph isn't working yet
  // quick and dirty cleanup so that I can watch it in action before I figure that out -3-
  if (event.key == 'k') {
    project.activeLayer.removeChildren()
    kanjiMorph = new KanjiMorph(kanjiMorph.targetKanji.char)
  }
