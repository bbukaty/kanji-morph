...
const KanjiSvg = window.KanjiSvg;
...
const bigKanji = "儲叢嚔嚠嚢壘壙彝懣懲懴戳擲擴擶擺擽擾攅斃斷旛曜檮檳檸櫁櫂櫃歸殯濫濺濾瀁瀉瀋瀏瀑燹燻燼燿獵璧瓊甓甕癒癖癜瞬瞹瞻瞼瞽瞿矇礇礎礑礒禮穡穢穣穫竄竅簟簡簣簧糧織繕繖繙繚繝繞繦繧繭羂羃翹翻聶職臍臑臨艟藍藏藕藜藝藤藥藩藪蟠蟯蟲襖襟襠覆覲観觴謦謫謬謳謾豐贄贅贈蹕蹙蹟蹠蹣蹤轆轉轌邃邇醪醫釐鎌鎔鎖鎗鎚鎧鎬鎭鎮鎰鎹闔闕闖闘雙雛雜難霤鞣鞦鞨鞫鞭鞳韓顋題額顎顏顔顕類餬餮馥騅騎騏騒験髀鬆鬩魍魎魏鮹鯀鯆鯉鯊鯏鯑鯒鵐鵑鵙鵜鵝鵞鵠鵤麌麿黠鼕鼬";
...
class KanjiMorph {
  ...
  morphTo(targetChar) {
    if (this.targetKanji != this.currentKanji) {
      console.log("please wait for current morph to finish.");
      return;
    }
  ...
  prepareMorph() {
    ...
    // use map for stroke mappings?
    for (let i = 0; i < this.currentKanji.strokes.length; i++) {
      let fromStroke = this.currentKanji.strokes[i];
      let toStroke = this.targetKanji.strokes[i]
      ...
      this.smoothJoinedPath(morphPath, fromStroke.segments.length - 1, 80);
      morphPath.visible = false;
      
      this.morphPaths.push(morphPath);
      this.morphStrokes.push(fromStroke.clone())
  ...
  morph(progress) {
    // linear
    let progressLinear = progress
    // sine based
    let progressSin = (Math.sin(Math.PI * progress + (Math.PI * 1.5)) + 1) / 2

    for (let i = 0; i < this.morphPaths.length; i++) {
      let startStroke = this.currentKanji.strokes[i];
      let endStroke = this.targetKanji.strokes[i];
      let morphPath = this.morphPaths[i];

      // stroke "tail" starts at 0 on the morph path, ends at end of endStroke
      let tailCurveOffset = (morphPath.length - endStroke.length) * progressSin;
      // interpolate length of morphStroke from startLength to endLength over progress
      let currLen = startStroke.length + (endStroke.length - startStroke.length) * progressSin;

      let startToTail = morphPath.clone();
      let morphStroke = startToTail.splitAt(tailCurveOffset);
      let headToEnd = morphStroke.splitAt(currLen);
      startToTail.remove();
      headToEnd?.remove();

      this.morphStrokes[i].remove();
      this.morphStrokes[i] = morphStroke;
    }
  }
  ...
  finalizeMorph() {
    ...
    console.log("done")
    // setTimeout(() => {
    //   let randomKanji = bigKanji[Math.floor(Math.random() * bigKanji.length)]
    //   this.morphTo(randomKanji);
    // }, 700);
  }
  ...
  smoothJoinedPath(path, joinIndex, minHandle) {
    let firstStrokeEnd = path.segments[joinIndex]
    firstStrokeEnd.handleOut = firstStrokeEnd.handleIn * -1
    firstStrokeEnd.handleOut.length = Math.max(firstStrokeEnd.handleOut.length, minHandle)

    let nextStrokeStart = path.segments[joinIndex + 1]
    nextStrokeStart.handleIn = nextStrokeStart.handleOut * -1
    nextStrokeStart.handleIn.length = Math.max(nextStrokeStart.handleIn.length, minHandle)
  }
}

// createGrid();
// let kanjiMorph = new KanjiMorph('丿'); // 1
let kanjiMorph = new KanjiMorph('二'); // 2
// let kanjiMorph = new KanjiMorph('題'); // 18
...
function onKeyDown(event) {
  if (event.key == 'space') {
    // kanjiMorph.morphTo('一'); // 1
    kanjiMorph.morphTo('人'); // 2
    // kanjiMorph.morphTo('藤'); // 18
  }
